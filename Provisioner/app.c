/***************************************************************************//**
 * @file
* @brief Silicon Labs Bluetooth Mesh Provisioner application (derived from
* embedded_provisioner example), built from BTMESH_SOC_EMPTY example.
 *******************************************************************************
 * # License
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_external_signal_id: {
      if(evt->data.evt_system_external_signal.extsignals & EX_B0_PRESS) {
        ...
        if (sc != SL_STATUS_OK) {
            app_log("Scan unprovisioned beacons fail: 0x%.2lx\r\n", sc);
        } else {
            app_log("PB0 Pressed, scanning for unprovisioned beacons...\r\n");
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_btmesh_evt_prov_initialized_id: {
      ...
      if (evt->data.evt_prov_initialized.networks == 0) {
        // No network present, create a new one
        ...
        // Network already exists, use the existing network id
        app_log("Network already exists, using existing network id = %x\r\n", network_id);
        app_log("Appkey already exists, using existing appkey index = %x\r\n", appkey_index);
      }
...
//      sl_btmesh_generic_client_init();
        ...
        if(0 > dev_idx) {
          ...
          for(idx = 0; idx < MAX_NUM_BTMESH_DEV; idx++) {
            if(0x00 == bluetooth_device_table[idx].address.addr[0] &&
              ...
              app_log("Address: 0x%x:0x%x:0x%x:0x%x:0x%x:0x%x\r\n", bluetooth_device_table[idx].address.addr[0],
              ...
              app_log("Net id: %d\r\n", network_id);
              app_log("UUID: ");
              for(uint8_t i = BLE_MESH_UUID_LEN_BYTE; i > 0; i--) app_log("0x%.2x:", bluetooth_device_table[idx].uuid.data[i - 1]);
              app_log("\r\n");
    ...
    case sl_btmesh_evt_prov_device_provisioned_id:
      ...
      app_log("UUID: ");
      for(uint8_t i = BLE_MESH_UUID_LEN_BYTE; i > 0; i--) app_log("0x%.2x:", provisionee_uuid.data[i - 1]);
      app_log("\r\n");
    ...
    case sl_btmesh_evt_config_client_binding_status_id:
      ...
      if(result == SL_STATUS_OK) {
        ...
        if(_sConfig.num_bind_done < _sConfig.num_bind) {
          ...
          model_id = _sConfig.bind_model[_sConfig.num_bind_done].model_id;
          vendor_id = _sConfig.bind_model[_sConfig.num_bind_done].vendor_id;
          if (vendor_id == MY_VENDOR_SERVER_ID || vendor_id == MY_VENDOR_CLIENT_ID) {
          ...
          app_log("All models are binded, setting publication\r\n");
          lcd_print("Model binded", SL_BTMESH_WSTK_LCD_ROW_STATUS_CFG_VAL);
          ...
          sc = sl_btmesh_config_client_set_model_pub(network_id,
                                                     ...
                                                     0, /* Publication time-to-live value */
                                                     0, /* period = NONE */
      ...
      if(result == SL_STATUS_OK) {
        ...
        if(_sConfig.num_pub_done < _sConfig.num_pub) {
          ...
          ** get the next model/address pair from the configuration list: */
          ...
          app_log("Publication setting are done, setting subscription\r\n");
          lcd_print("PUB set", SL_BTMESH_WSTK_LCD_ROW_STATUS_CFG_VAL);
    ...
    case sl_btmesh_evt_config_client_model_sub_status_id:
      ...
      if(result == SL_STATUS_OK) {
        ...
        if(_sConfig.num_sub_done < _sConfig.num_sub) {
          ...
          app_log("\r\nLong press BTN0 to provision the first seen Node. Long press BTN1 to list the unprovisioned nodes for targeted provisioning.\r\n\r\n");
          lcd_print("Provision done", SL_BTMESH_WSTK_LCD_ROW_STATUS_CFG_VAL);
        }
      } else {
    ...
    case sl_btmesh_evt_prov_start_sent_id:
      app_log("Provisioning node: %02x\r\n", evt->data.evt_prov_start_sent.uuid.data[0]);
      break;
    case sl_btmesh_evt_prov_capabilities_id:
      app_log("UUID of device to be provisioned: %02x\r\n", evt->data.evt_prov_capabilities.uuid.data[0]);
      break;
...
void provision_bluetooth_device(device_table_entry_t *device)
{
  ...
  if(sc == SL_STATUS_OK) {
    ...
    app_log("Provisioning fail %lX:\r\n", sc);
  }

  app_log("Net id: %d\r\n", network_id);
  app_log("UUID: ");
  for(uint8_t i = BLE_MESH_UUID_LEN_BYTE; i > 0; i--) app_log("0x%.2x:", device->uuid.data[i - 1]);
  app_log("\r\n");
...
void print_bluetooth_device_table()
{
  ...
  for(uint8_t dev_idx = 0; dev_idx < MAX_NUM_BTMESH_DEV; dev_idx++) {
    ...
    if((0x00 != bluetooth_device_table[dev_idx].address.addr[0] ||
      ...
      app_log("Index: %d - ", dev_idx);
      app_log("Address: 0x%x:0x%x:0x%x:0x%x:0x%x:0x%x\r\n", bluetooth_device_table[dev_idx].address.addr[0],
  ...
  app_log("\n\rSelect device [Index] with BTN0/1 - Press BTN0 for more than 1s to provision it:\r\nIndex:  ");
  return;
}
...
static void config_check()
{
  ...
  for(int i = 0; i < _sDCD_Prim.numSIGModels; i++) {
      // Process for SIG Models
  }
  for(int i = 0; i < _sDCD_Prim.numVendorModels; i++) {
      // Process for SIG Models
    if(_sDCD_Prim.vendor_models[i].model_id == MY_VENDOR_SERVER_ID) {
        config_pub_add(MY_VENDOR_SERVER_ID, VENDOR_ID, CUSTOM_STATUS_GRP_ADDR);
        config_sub_add(MY_VENDOR_SERVER_ID, VENDOR_ID, CUSTOM_CTRL_GRP_ADDR);
        config_bind_add(MY_VENDOR_SERVER_ID, VENDOR_ID);
    }
    if(_sDCD_Prim.vendor_models[i].model_id == MY_VENDOR_CLIENT_ID) {
        config_pub_add(MY_VENDOR_CLIENT_ID, VENDOR_ID, CUSTOM_CTRL_GRP_ADDR);
        config_sub_add(MY_VENDOR_CLIENT_ID, VENDOR_ID, CUSTOM_STATUS_GRP_ADDR);
        config_bind_add(MY_VENDOR_CLIENT_ID, VENDOR_ID);
    }
    if(_sDCD_Prim.vendor_models[i].model_id == MY_VENDOR_RELAY_ID) {
        config_pub_add(MY_VENDOR_RELAY_ID, VENDOR_ID, CUSTOM_CTRL_GRP_ADDR);
        config_sub_add(MY_VENDOR_RELAY_ID, VENDOR_ID, CUSTOM_CTRL_GRP_ADDR);
        config_bind_add(MY_VENDOR_RELAY_ID, VENDOR_ID);
    }
  }
}
  ...
  switch (duration) {
    case APP_BUTTON_PRESS_DURATION_SHORT:
      ...
      if(button == BUTTON_PRESS_BUTTON_0) {
        ...
        else select_provisionee_target = 0;
      } else {
        if(select_provisionee_target > 0) select_provisionee_target--;
        else select_provisionee_target = max_provisionee_index;
      }
  ...
  switch (duration) {
    ...
    case APP_BUTTON_PRESS_DURATION_LONG:
      ...
      if(button == BUTTON_PRESS_BUTTON_0) {
        ...
        if (unprov_dev == 0) {
            app_log("No unprovisioned devices!\r\n");
        } else {
